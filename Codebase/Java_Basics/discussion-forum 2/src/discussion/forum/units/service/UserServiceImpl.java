...
public class UserServiceImpl implements UserService {
  ...
  public User createUser(String username, String password, String email, UserRole userRole) {
    if (Utility.isNotNullAndEmpty(username)
        && Utility.isNotNullAndEmpty(email)
        && Utility.isNotNullAndEmpty(password)
        && (userRole != null)) {
  ...
  private User getUser(String username) {
    for (User user : users) {
      if (user.getUsername().equals(username) || user.getEmail().equals(username)) return user;
    }
    return null;
  }
}
