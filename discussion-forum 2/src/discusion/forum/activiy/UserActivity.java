...
public class UserActivity {
  ...
  public User loginActivity() throws IOException {
    ...
    System.out.println(
        "You do not have the account. Request admin to get account in discussion forum");
    return null;
  }
  ...
  public void postNewQuestion(User user) throws IOException {
    ...
    do {
      ...
      title = Utility.inputFromUser();
      if (!Utility.validInputString(title)) {
        System.out.println("Invalid question title. Please enter valid question title");
      }
    } while (!Utility.validInputString(title));
    ...
    do {
      ...
      if (!Utility.validInputString(message)) {
        System.out.println("Invalid question body. Please enter valid question body");
      }
    } while (!Utility.validInputString(message));
   ...
   * @throws IOException
   */
  public void seeAllQuestions(UserActivity userActivity, User user)
      throws NumberFormatException, IOException {
    ArrayList<Question> questions = questionService.getQuestions(); // getting all the questions

    // ** Please write your code below: **
    //
    // Replace the ... in the if statement condition with correct boolean statement. You should
    ...
    //
    // You can check how many the number of questions by using the size() function on the questions
    // variable
    ...
    Collections.sort(
        ...
        new Comparator<Question>() {
          public int compare(Question q1, Question q2) {
            if (q1.getUpvoteCount() == q2.getUpvoteCount()) return 0;
   ...
   * @throws IOException
   */
  public void deleteQuestion(UserActivity userActivity, User user)
      throws NumberFormatException, IOException {
    ...
    // ** Please write your code below: **
    //
    // Note: This is the most challenging part of the project, so please don't worry if you don't
    // get it correct
    // on the first try. Please feel free to look at the solution if you are stuck.
    //
    // Write the logic to delete a question from the class forum, and you should follow the rules
    // below:
    ...
    //
    // 3) Else if the current user is also the user who wrote the question, we can delete the
    // question.
    ...
    if (questionService.getQuestions().size() == 0) DiscussionForum.menu(user, userActivity);
  }
  ...
  public void seeAllReplies(UserActivity userActivity, User user)
      throws NumberFormatException, IOException {
